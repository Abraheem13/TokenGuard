...
\item \textbf{C2: an explanation that predicts.} Proposition~\ref{prop:spurious} attributes spurious agreement to the persistence of incorrect answers; its prediction was confirmed out-of-sample on a benchmark added after the proposition was stated (Section~\ref{sec:rq2}).
...
Within that literature, adaptive computation has a long history in encoder models, where classifiers attached to intermediate layers permit a forward pass to terminate early~\cite{xin2020deebert,liu2020fastbert,zhou2020bert,elbayad2020depth,han2021dynamic}, and in decoders through confident adaptive decoding~\cite{schuster2022confident} and speculative execution~\cite{leviathan2023fast}. Early exit during \emph{reasoning} differs from all of these in one respect that matters for the present study: the unit of decision is a semantic checkpoint in an argument rather than a layer or a token, so the quantity being monitored is a belief about an answer rather than a property of a computation. DEER interrupts at reasoning transitions, forces a trial answer and halts on its confidence~\cite{yang2025deer}; Certaindex halts when successive trial answers agree and uses that signal to schedule a serving system~\cite{fu2024certaindex}; EAT appends a stop-thinking token and thresholds the variance of the entropy that follows~\cite{wang2025eat}; MUR compares step-level negative log-likelihood against its own momentum to decide where extra computation is warranted~\cite{yan2025mur}; REFRAIN adapts a confidence threshold across a query stream with a sliding-window bandit~\cite{sun2025refrain}; and budget-aware or length-controlled methods either impose a quota or learn one~\cite{han2024token,muennighoff2025s1,aggarwal2025l1,xu2025chain,ma2025reasoning,zhang2025adaptthink}. Each of these commits to a single fixed rule, which is precisely the design decision this paper examines. Our signal library reimplements the first five at the level of the signal, under one shared probing protocol, so that what is compared is the notion of ``finished'' rather than the surrounding engineering.
...
\midrule
$k, K$ & checkpoint index; number of checkpoints in a trace \\
$t_k, p_k$ & thinking tokens at checkpoint $k$; tokens spent on probe $k$ \\
$\hat a_k$ & forced trial answer at checkpoint $k$ \\
$c_k, S_k$ & trial-answer confidence; its exponentially smoothed value \\
$H_k, M_k, r_k$ & first-token entropy; NLL momentum; agreement run length \\
$\pi_0$ & null action: never halt \\
$\mathcal{C}, \mathcal{F}$ & candidate library; certified (feasible) subset \\
$\delta_c, \Delta_c$ & paired per-item accuracy difference; its expectation \\
$\varepsilon, \alpha$ & accuracy budget (SLO); confidence level of the certificate \\
$b, A(b)$ & cost budget as a fraction of full thinking; achievability curve \\
$\rho_w, q_w$ & persistence and modal mass of \emph{incorrect} trial answers \\
\bottomrule
\end{tabular}
\end{table}
...
\intuit{Do not deploy the option that merely looks best on a small trial. Deploy the cheapest option that is \emph{provably} good enough, after allowing for the fact that trying many options makes some of them look good by luck. If nothing passes that test, do not gamble --- keep thinking.}
...
\subsection{Metrics}
Because early exit trades accuracy against cost, a bare pair of numbers cannot establish superiority; the comparison is meaningful only once one axis is held fixed. We therefore fix the metrics before measurement. Each policy traces an \emph{achievability curve} over its own hyper-parameter $k$,
\begin{equation}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_collapse.pdf}
...
Table~\ref{tab:slo} audits the guarantee on held-out data at $\alpha=0.10$. The fusion tier remains within one accuracy point in every setting, with a worst case of $-0.5$ points; the risk-controlled selection tier is within one point in $82\%$ of settings with a worst case of $-1.6$, and within the relaxed $2.5$-point budget in all of them. Every fixed signal has a far heavier tail, reaching $-23.6$ points. Efficiency and safety therefore arise from the same design rather than trading against one another. The difference between the two families is not one of degree: a method whose worst case is $-23.6$ points cannot be deployed behind a service-level agreement no matter how good its average.
...
That is precisely the trade this paper is about. DEER's accuracy is bought with a trial-answer overhead of $66$--$79\%$ of the final chain across these six settings ($54$--$80\%$ across all nine runs we executed; on MATH-500-4B, $3{,}539$ total tokens against $2{,}043$ in the final chain), against the $0.9$--$7.1\%$ measured in Section~\ref{sec:rq5}. A method that pays two-thirds of a chain in probes is viable under KV-fork accounting and is the first casualty of a black-box regime. The contribution here is not a better fixed operating point than DEER's, but a controller that does not collapse when the workload changes and that states what it guarantees; the confidence signal DEER monitors, evaluated across the eleven settings of Table~\ref{tab:slo} under our protocol, loses up to $7.5$ points.
