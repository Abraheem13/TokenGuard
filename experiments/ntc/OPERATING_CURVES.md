# Operating curves under the oracle and deployable protocols

One calibration/evaluation split (seed 0) per setting. `oracle`: each rule's parameter is chosen per budget on the evaluation items (an upper bound, not realisable in deployment). `deployable`: each rule chooses its parameter from the calibration items. Both protocols are scored on the same evaluation items with overhead-inclusive costs. `n/a`: no parameter reaches that budget.

## gsm8k / Qwen3-4B: evaluation n=120, full-generation accuracy 0.900

**Oracle protocol (parameter chosen on the evaluation items)**

| method | AUCC | coverage | A(b=0.5) | B*(1 pt) |
|---|---|---|---|---|
| NTC-v2 (fusion) | 0.950 | 100% | 0.942 | 0.33 |
| Answer agreement | 0.938 | 100% | 0.933 | 0.44 |
| NTC-full (selection) | 0.931 | 100% | 0.933 | 0.33 |
| Smoothed confidence | 0.924 | 100% | 0.925 | 0.29 |
| Entropy (EAT) | 0.908 | 86% | 0.908 | 0.47 |
| Confidence (DEER-λ) | 0.864 | 100% | 0.867 | 1.00 |

**Deployable protocol (parameter chosen on the calibration items)**
...
## gsm8k / Qwen3-8B: evaluation n=120, full-generation accuracy 0.842
...
| NTC-full (selection) | 0.917 | 100% | 0.917 | 0.31 |
...
## math500 / Qwen3-1.7B: evaluation n=120, full-generation accuracy 0.767
...
| NTC-full (selection) | 0.707 | 100% | 0.658 | 0.87 |
...
## math500 / Qwen3-4B: evaluation n=120, full-generation accuracy 0.783
...
| NTC-full (selection) | 0.743 | 100% | 0.675 | 0.54 |
...
## math500 / Qwen3-8B: evaluation n=120, full-generation accuracy 0.758
...
| NTC-full (selection) | 0.805 | 100% | 0.808 | 0.48 |
...
## gpqa_diamond / Qwen3-4B: evaluation n=119, full-generation accuracy 0.538
...
| NTC-full (selection) | 0.463 | 100% | 0.420 | 1.00 |
...
## gpqa_diamond / Qwen3-8B: evaluation n=119, full-generation accuracy 0.563
...
| NTC-full (selection) | 0.495 | 100% | 0.437 | 0.97 |
...
## mmlu_pro / Qwen3-4B: evaluation n=120, full-generation accuracy 0.683
...
| NTC-full (selection) | 0.608 | 100% | 0.550 | 1.00 |
...
## mmlu_pro / Qwen3-8B: evaluation n=120, full-generation accuracy 0.733
...
| NTC-full (selection) | 0.693 | 100% | 0.642 | 0.91 |
...
## aime24 / Qwen3-4B: evaluation n=18, full-generation accuracy 0.667

**Oracle protocol (parameter chosen on the evaluation items)**

| method | AUCC | coverage | A(b=0.5) | B*(1 pt) |
|---|---|---|---|---|
| NTC-full (selection) | 0.611 | 57% | n/a | 0.90 |
...
| Entropy (EAT) | 0.444 | 71% | n/a | 1.00 |
| NTC-v2 (fusion) | 0.413 | 100% | 0.222 | 0.96 |
| Answer agreement | 0.397 | 100% | 0.222 | 1.00 |
## aime25 / Qwen3-4B: evaluation n=18, full-generation accuracy 0.500

**Oracle protocol (parameter chosen on the evaluation items)**

| method | AUCC | coverage | A(b=0.5) | B*(1 pt) |
|---|---|---|---|---|
| NTC-full (selection) | 0.537 | 43% | n/a | 0.71 |
| Smoothed confidence | 0.478 | 71% | n/a | 0.72 |
...
| Smoothed confidence | 0.422 | 71% | n/a | 1.00 |
| Entropy (EAT) | 0.370 | 86% | 0.278 | 0.92 |
| Confidence (DEER-λ) | 0.341 | 100% | 0.167 | 1.00 |
## math500 / DeepSeek-R1-Distill-Qwen-7B: evaluation n=120, full-generation accuracy 0.892
...
| NTC-full (selection) | 0.781 | 86% | 0.625 | 0.89 |
...
`worst Δ accuracy` is the lowest accuracy change, over settings, of the point each rule deploys at a one-point tolerance (eps = 0.01).

| method | mean AUCC | min AUCC | worst Δ accuracy (pts) |
|---|---|---|---|
| NTC-full (selection) | 0.691 | 0.463 | -3.3 |
